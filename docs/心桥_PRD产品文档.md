...
> 文档版本：V3.0（完整详尽版，依据当前代码实现逐项核对整理）
> 整理日期：2026-08-05
> 产品名称：心桥｜品牌口号：为你搭建与客户之间心的桥梁
...
1. **减少遗漏**：提前 7 天提醒 + 当天提醒，客户经理不再依赖人工记忆；
...
```
客户信息管理 → 生日自动识别 → 提前提醒 → 生日当天提醒 → 完成客户维护 → 沉淀维护记录
```
...
| 生日提醒 | FR-26～FR-30 | 提前 7 天、当天 A 类三次、状态流转、开关、卡片跳详情 | 已实现 |
...
| 客户简称 | 是 | 刘先生 | 非空 |
| 生日 | 是 | 1988-08-20 或 08-20 | 可解析的真实日期 |
| 性别 | 否 | 男/女/未知 | 枚举 |
| 行业 | 否 | 制造业 | 枚举 8 类 |
...
- 当天提醒：仅 A 类客户，09:00 / 10:00 / 14:00 三个时间点（应用内展示，无系统推送）；
- 状态流转：待联系 →（标记已联系）→ 已联系；“已联系/已完成”由当天是否存在维护记录**派生**，避免跨年状态过期；
- 提醒开关本地持久化（localStorage，key：`birthday-care.settings.v1`）。
...
| 提醒 | 底部导航“提醒” | 提前 7 天、当天提醒、维护记录 |
| 设置 | 底部导航“设置” | 数据管理、提醒设置、隐私说明、创作者标识 |
...
| 优先级 | 范围 | 状态 |
| --- | --- | --- |
| P0 | 客户新增/列表/Excel 导入、生日计算、首页提醒、当天提醒 | 已完成 |
...
10. **隐私**：无后端、无埋点；敏感字段不入库；
11. **部署**：Direct Upload 多段上传易踩坑，优先使用 Wrangler CLI（`wrangler pages deploy`）而非手写 multipart。
