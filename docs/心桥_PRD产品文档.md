...
- **家属关系模块**：关系类型徽标（夫妻/子女/父母/其他）+ 姓名；支持按客户编号/姓名搜索添加关联客户（姓名可点击跳转到对方详情页，并显示“已关联客户”），也可仅登记姓名与备注；支持新增、编辑、删除（删除需确认）；客户编辑弹窗内同样可维护家属关系；首页今日卡片、客户列表行、提醒时间线均可点击进入客户详情；
...
步骤：点击“清空数据” → 弹窗第一次确认“清空所有数据？”（按钮：继续）→ 第二次确认“再次确认” → 第三次“最后确认”（红色按钮“确认清空”）→ 执行删除。删除范围：全部客户 + 全部维护记录 + 全部家属关系 + 导入会话；删除不可恢复。
...
导出 .xlsx，含“客户”“维护记录”“家属关系”三个工作表，客户列与导入模板一致。
