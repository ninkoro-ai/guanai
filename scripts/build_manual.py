...
def build():
    ...
    h1(doc, "1  产品简介")
    h2(doc, "1.1  产品定位")
    para(doc, "心桥是一款面向银行客户经理的客户关怀系统，品牌 slogan 为“您与客户之间心的桥梁”，常驻展示在应用顶部标题区。产品以“客户编号 + 生日提醒 + 祝福辅助 + 维护记录 + 家属关系”为主线，帮助客户经理每天用 30 秒到 2 分钟完成重点客户的关怀，避免遗漏、沉淀关系资产。")
    h2(doc, "1.2  核心功能一览")
    for item in [
    ...
    para(doc, "顶部标题区展示品牌信息与个人问候：左侧为“心桥 · 客户关怀系统”与品牌 slogan“您与客户之间心的桥梁”；中间居中显示当天日期；右侧为应用图标与“Hi，用户名”问候。用户名可在“设置 → 账户与团队”中填写，未设置时显示“Hi，客户经理”。")
    h2(doc, "3.1  顶部统计")
    ...
    para(doc, "设置页按类别分组展示，共四组：账户与团队、数据管理、提醒设置、帮助与关于。")
    h2(doc, "8.1  账户与团队")
    ...
    h2(doc, "8.5  帮助与关于")
    para(doc, "用户手册（PDF）：点击“查看 / 下载”可打开最新版完整使用手册，包含产品定位（客户关怀系统）、品牌 slogan（您与客户之间心的桥梁）与全部功能介绍；PDF 为独立文件，通过链接打开或下载，不占用应用本身体积。")
    para(doc, "隐私说明：仅保存客户编号、简称、生日、性别、行业、等级与备注；不保存身份证号、账号、完整姓名与交易信息；数据仅存于本机浏览器，不上传服务器。")
    para(doc, "关于心桥：心桥 · 客户关怀系统——您与客户之间心的桥梁。面向银行客户经理，提供生日提醒、祝福辅助、维护记录与家属关系管理。")
    add_note(doc, "设计考量", "设置项按“账户 / 数据 / 提醒 / 帮助”分组，让高频操作（导出、导入）与低频操作（清空、关于）各归其位。用户名与团队为后续日报汇总铺路，Header 问候则让工具更有“自己的”感觉。用户手册以 PDF 形式外链查看，保持应用轻量化。")
