...
def build():
    ...
    h2(doc, "8.5  帮助与关于")
    para(doc, "用户手册（PDF）：点击“查看 / 下载”可打开最新版完整使用手册，包含产品定位（客户关怀系统）、品牌 slogan（心的桥梁）与全部功能介绍；PDF 为独立文件，通过链接打开或下载，不占用应用本身体积。")
    para(doc, "隐私说明：仅保存客户编号、简称、生日、性别、行业、等级与备注；不保存身份证号、账号、完整姓名与交易信息；数据仅存于本机浏览器，不上传服务器。")
    para(doc, "关于心桥：心桥 · 客户关怀系统——心的桥梁。面向银行客户经理，提供生日提醒、祝福辅助、维护记录与家属关系管理。")
